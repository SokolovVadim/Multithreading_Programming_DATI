...
int main()
{
	...
	if(sem_init(&resource,   0, 1) == -1)
		handle_error("sem_init");
	if(sem_init(&rtry,   0, 1) == -1)
		handle_error("sem_init");
	if(sem_init(&wentry,   0, 1) == -1)
		handle_error("sem_init");

	std::vector<std::thread> threads(4 * THREAD_NUM);
	...
	for(int i(2 * THREAD_NUM); i < 3 * THREAD_NUM; ++i)
		threads[i] = std::thread(reader, i);

	for(int i(3 * THREAD_NUM); i < 4 * THREAD_NUM; ++i)
		threads[i] = std::thread(writer, i);
	...
	if(sem_destroy(&resource) == -1)
    	handle_error("sem_destroy");
    if(sem_destroy(&rtry) == -1)
    	handle_error("sem_destroy");
	if(sem_destroy(&wentry) == -1)
    	handle_error("sem_destroy");
